...
`git clone https://github.com/ByteLegendQuest/java-filter-prime-numbers`

A project directory named `java-filter-prime-numbers` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-filter-prime-numbers` directory which is created just now.
